...
#import "SearchThingsViewController.h"
...
@implementation SearchThingsViewController
...
- (void)dataDidDownload:(NSNotification *)notification {
    ...
    if ([notification.object isEqualToString: ASYNC_EVENT_SEARCHTHINGS])
...
//    AppData *appData = [AppData sharedInstance];
    if ([[ret objectForKey:@"INFO"] isEqualToString:@"newest"]) {
        int count = PAGE_THINGS_COUNT >_tids.count ?_tids.count : PAGE_THINGS_COUNT;
        _data = [[NSMutableArray alloc] initWithArray:[_tids subarrayWithRange:NSMakeRange(0, count)]];
...
//   _seq = [data objectForKey:@"seq"];
   ...
   _tids = [NSMutableArray arrayWithArray:[[tids reverseObjectEnumerator]allObjects]];
    //    int index = 0;
    //    if (_tids.count)
    //        index = [tids indexOfObject:[_tids objectAtIndex:0]];
    //    LOG(@"index %d", index);
    //    if (index == NSNotFound) index = 0;
    //    for (int i = index + 1; i < tids.count; i++)
    //        [_tids insertObject:[tids objectAtIndex:i] atIndex:0];
//    [appData.myThingsLine update];
    int count = PAGE_THINGS_COUNT >_tids.count ?_tids.count : PAGE_THINGS_COUNT;
    [FindLogic downloadThingsInfo:[_tids subarrayWithRange:NSMakeRange(0, count)]from:CLASS_NAME];
    [self.tableView reloadData];
...
- (void)thingsInfoDidDownload:(NSNotification *)notification {
    ...
    if (_downloadFromTop)
        ...
        _maxLoadedPage++;
        count = PAGE_THINGS_COUNT * (_maxLoadedPage+1) >_tids.count ?_tids.count : PAGE_THINGS_COUNT * (_maxLoadedPage+1);
        _data = [[NSMutableArray alloc] initWithArray:[_tids subarrayWithRange:NSMakeRange(0, count)]];
        _backgroundWorking = NO;
    }
    _downloadFromTop = NO;
...
//    AppData *appData = [AppData sharedInstance];
    int count =_tids.count - (_currentPage + 1) * PAGE_THINGS_COUNT;
    if (count > PAGE_THINGS_COUNT) count = PAGE_THINGS_COUNT;
    if (count < 0) return;
    _backgroundWorking = YES;
    [FindLogic downloadThingsInfo:[_tids subarrayWithRange:NSMakeRange(PAGE_THINGS_COUNT * (_currentPage + 1), count)]from:CLASS_NAME];
